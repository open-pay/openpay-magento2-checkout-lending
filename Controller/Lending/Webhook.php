...
class Webhook extends \Magento\Framework\App\Action\Action implements CsrfAwareActionInterface
{
    ...
    public function execute() {
        ...
        try {
            ...
            if (isset($json->type) && ($json->transaction->method == 'lending')) {
                ...
                if($json->type == 'charge.succeeded' && $charge->status == 'completed' ){
                    ...
                    $order->addStatusHistoryComment("Confirmación de pago recibido exitosamente (Webhook)")->setIsCustomerNotified(true);
                    $order->save();
                    ...
                    $invoice->register();
                    $invoice->save();
                // TRANSACCION EXPIRADA O RECHAZADA
                }else if($json->type == 'transaction.expired' || $charge->status == 'rejected'){
